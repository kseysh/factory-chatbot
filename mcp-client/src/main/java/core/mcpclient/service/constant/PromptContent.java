...
public enum PromptContent {
    ...
    SYSTEM_PROMPT_DEFAULT_CHAT(
            ...
            ### Tone & Format
            - 전문적이되 대화체를 유지합니다. (해요체 또는 하십시오체)
            - 간결하고 명확하게 작성합니다.
            - Markdown 형식(특히 굵은 글씨, 제목, 목록, 표 등)을 사용하지 마십시오.
            ---
            """
    ),
    SYSTEM_PROMPT_CREATE_NEW_CHAT(
            ...
            **주의사항 (엄격히 준수):**
            ...
            [출력 예시]
            전력 사용량 증가 원인 분석§최근 전력 사용량이 증가한 원인은 냉난방 설비 가동 시간 증가와 외부 기온 변화에 따른 부하 상승 때문이에요. 특히 오후 시간대에 사용량이 집중되는 경향이 있어, 해당 시간대의 설비 운전 패턴을 점검하면 에너지 절감에 도움이 될 수 있어요.
