...
class SnowflakeTest {
	...
	@Test
	@DisplayName("모든 id가 오름차순으로 생성되는지 확인")
	void nextIdTest() throws ExecutionException, InterruptedException {
		...
		for (Future<List<Long>> future : futures) {
			...
			for (int i = 1; i < idList.size(); i++) {
				assertThat(idList.get(i)).isGreaterThan(idList.get(i - 1)); // id가 오름차순으로 생성되는지 확인
			}
			result.addAll(idList);
		}
		// 모든 id가 중복 없이 정상적으로 생성되었는지 확인
	...
	@Test
	@Disabled("Assertion이 존재하는 Test가 아님")
	@DisplayName("id 생성 시간 체크")
